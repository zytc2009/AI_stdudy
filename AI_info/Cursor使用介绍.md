...
有问题先查文档：https://cursor.com/cn/docs
...
Cursor的规则系统通过项目规则（Project Rules ，版本控制）、用户规则（User Rules，全局偏好）和智能回忆（Memory，自动生成）三大机制，为AI行为提供持久化指引，解决大模型的「记忆断层」问题，确保代码生成、编辑建议始终符合预设规范。
1.项目规则 → 团队共享的代码库标准 
2.用户规则 → 个人开发习惯（全局生效） 
3.回忆规则 → 自动从历史对话提取高频指令

Cursor规则推荐: 新手GitHub、老手cursor.directory，企业AgentDesk

```
https://github.com/PatrickJS/awesome-cursorrules  //找对应语言的rules文件
```

项目规则以 markdown 文件形式存放在 `.cursor/rules` 中，并纳入版本控制
...
“Cloud Agent” 和 “Remote SSH” 是两项非常强大但功能完全不同的技术。

或者做个本地httpserver，通过终端或网页发指令

## 七、官方推荐的技巧

1. 先定规矩很重要 就像玩游戏先要看说明书一样，用 Cursor 前先定好项目规则。敲个“/generate rules”命令，或者去 GitHub 上找现成的规则模板。这样 AI 就不会瞎编乱造，生成的代码更符合你的需求。 

2.  把需求说清楚 别只说“写个登录页面”这么笼统，要像点菜一样详细：“用 React 最新版+TypeScript 写个登录页面，要有邮箱验证、密码强度检查，登录成功跳首页，失败要弹提示”。这样 AI 才能给你想要的代码。 

3.  拆开慢慢来 别想着一口吃成胖子，把大功能拆成小模块。先写用户模型，测试通过了再写服务层，最后写控制器。稳扎稳打才靠谱。 

4.  先写测试再写代码 这是个好习惯!先让 AI 帮你把测试用例写好，比如“测试空用户名应该报错”，等测试都通过了再写实现代码，省得后面改来改去。 

5.  错了要教 AI 发现 AI 写错了别光顾着改，要告诉它错在哪、怎么改。就像教小朋友一样，教过一次下次就会了。 

6.  精准定位文件 用@file 指令指定具体文件，比如“@file src/login.js 把按钮颜色改成蓝色”，这样 AI 就不会改错地方。 

7.  做好任务管理 在.cursor 文件夹里放个 to-do list，要把做的功能一条条列清楚。做完一条划掉一条，井井有条。 

8.  复杂代码自己写 遇到特别复杂的逻辑，别跟 AI 较劲，自己动手写更快。你写的代码 AI 会偷偷学，下次就能写得更好。 

9.  聊天记录要整理 给重要对话起个好记的名字，比如“用户登录功能讨论”，方便以后查找。回退代码还是用 Git 更靠谱。 

10. 对 AI 模型 精细活选 Gemini，赶时间选 Claude。拿不准就选 auto，让 AI 自己看着办。 

11. 多看官方文档 学新框架时，把官网链接扔给 AI:“这是 Vue3 官网，照着这个写个计数器”，比自己查文档快多了。 

12. 项目要耐心 处理大项目时，先让 AI 把整个项目“消化”一下，等它准备好了再开工，磨刀不误砍柴工。

    

## 总结建议

![](images\cursor总结.jpeg)
